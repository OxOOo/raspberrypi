...
async function mkdirp(pathname: string) {
    if (await mzfs.exists(pathname)) return;
...
export class D {
    ...
    private downloadToLocal() {
        return new Promise<void>(async (resolve, reject) => {
            try {
                ...
                await mkdirp(path.dirname(path.join(pathname, this.download.local_file)));
    ...
    private async analysisLocalFilename() {
        ...
        if (this.download.type == 'autoupdate') {
            this.download.local_file = path.join(this.download.version, filename);
        } else {
